...
export default class Engine {
    ...
    constructor() {
        ...
        this.executeTestCasesWithErrorDataSet9()
    ...
    peekPatterns(msg,lang,dimension){
        console.log(`Peeking pattersnf for memory[${lang}][${dimension}]`)
        let blackbox = this.memory[lang][dimension].archive.referencesUntransformedPatterns
        // console.log(blackbox.archive.referencesUntransformedPatterns)
        // let input = msg
        // this.memory[lang][dimension].getReplyUsingBM25(msg)
        // this.memory[lang][dimension].getReplyUsingTanimotoCoefficient(msg)
        
        // let input = "masakit samaya  ulo ngin gamot"
        blackbox.forEach(unit=>{
            let pattern = unit.pattern
            let score = this.memory[lang][dimension].getReplyUsingCosineSimilarity(msg)
            // let score = this.memory[lang][dimension].getReplyUsingTanimotoCoefficient(msg)
            // let score = tanimotoCoefficient("masakit samaya  ulo ngin gamot",unit.pattern)
            console.log(pattern)
            console.log(`Score: ${JSON.stringify(score)}`)
            
            // let patterns = unit.pattern
        })
    }

    testSimilarity(a,b){

        let score = tanimotoCoefficient(a,b)
        console.log('Score For : ', a , " against ", b, " = ", score)
    }
    ...
    buildSymmetricDifference(){
        ...
        let dia = [].concat(diarrheaEngRules,diarrheaFilRules,diarrheaMagRules)
        let flu = [].concat(influenzaEngRules,influenzaFilRules,influenzaMagRules)
        // ENG: [].concat(diarrheaEngRules,influenzaEngRules,genericEngRules),
        // FIL: [].concat(diarrheaFilRules,influenzaFilRules,genericFilRules),
        // MAG: [].concat(diarrheaMagRules,influenzaMagRules,genericMagRules)
    
        let buildTerms = function(data){
            let terms = []
            // build influenza terms 
            data.forEach(rule=>{
                ...
                if (Array.isArray(patterns)){
                    ...
                    patterns.forEach(p=>{
                        ...
                        // p = p.replace(/[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/g,' ').trim()
                        
                        // lets ignore "_" because we are using it
                        p = p.replace(/[`~!@#$%^&*()|+\-=?;:'",.<>\{\}\[\]\\\/]/g,' ').trim()
                        p = p.replace(
                            /(^\s+|\s+$)|\s+/g, 
                            function (match, leadingTrailing, duplicate) {
                        ...
                        words.forEach(w=>{
                            ...
                            let sub = DICTIONARY.getSubstitute(w)

                            if ( sub != null ){
                                // replace if substitute exist
                                terms.push(sub)
                            }else{
                                // if none the just push
                                terms.push(w)
                            }
        ...
        let influenzaTerms = buildTerms(flu)
        let diarrheaTerms = buildTerms(dia)

        // set UNION 
        // "... new Set() " not only merge but also removes repeating items
        let union = [...new Set([...diarrheaTerms, ...influenzaTerms])];
        console.log(`The union`)
        console.log(union)
        union = union.filter(word=>{ return !DICTIONARY.isStopWord(word) })

        // result is the accumulator while [] is the 
        let intersection = influenzaTerms.reduce((result, item) => {
            if ( diarrheaTerms.includes(item) && !result.includes(item)) {
              result.push(item);
            }
            return result;
          }, []);
        ...
        // filter intersection to remove stopwords
        intersection = intersection.filter(word=>{ return !DICTIONARY.isStopWord(word) })

        // symmetric difference = union - intersection        
        let symmetricResult = union.filter(word=>{
            // lets remove words from union that is 
            // actually present in the intersection
            return !intersection.includes(word)
        })

        console.log(`The Symmetric Difference`)
        // console.log(JSON.stringify(symmetricResult))
        console.log(symmetricResult)
    }
    ...
    getReply(msg) {
        ...
        let trimmed = msg.toLowerCase()
        trimmed = this.removeRedundantSpaces(trimmed)
        
        // WARNING:: YOU NEED TO ADD ANOTHER STEP TO REPLACE I'M DON'T BY REMOVING APOSTROPHES
        // DO THESE FOR THOSE WORDS
        
        // step 5
        let lang = this.classifierLanguage.getPrediction(trimmed)
        ...
        trimmed.split(' ').forEach(token=>{
            for ( let i = 0 ; i < COMMON_MISPELLED_WORDS.length; i++ ){
                ...
                if ( distance <= (Math.ceil(token.length/2))-1  && 
                    ...
                    spellChecked = spellChecked.replace(regex,word)
                }
            }
        })
        ...
        // console.log('xx::cleaned <<',cleaned)
        ...
        reconstructed = this.removeRedundantSpaces(reconstructed) 
        ...
        reconstructed.split(' ').forEach(token=>{
            if ( DICTIONARY.isStopWord(token.trim()) ){
                ...
                stopWordsRemovedInput = stopWordsRemovedInput.replace(regex, '')
            }
        })
