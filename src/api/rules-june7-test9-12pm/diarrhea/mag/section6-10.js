...
let section6 = [
    encodeMemory([
      ...
      `panon a to o ngin to ba ka nasisita su kaawa sa upis na unga bago kanen`,
      `panon a to ngin to ka dayt a upisan o bagawan sa upis su unga o pegken tupan na kanen`,
      `panon a to nginto ka upisan o awan sa upis muna i unga o pegken bago kanen.`,
      `panon a to ngin to ka awan muna sa upis i unga o pegken bago kanen`,
      `enduken ka pangupisan su mga pegken bago pegkanen`,
      `nginto ka nasisita su kaupis sa pegken bago pegkanen`,
    ],[
    ...
    encodeMemory([
      `* duken * di * kan * da naluto * karne * `,
      `* duken * di * kan * da maluto * karne * `,
      `* duken * di * kan * mailaw * karne * `,
      `* panon o enduken a to ka di keman su da maluto a karne`,
      ...
      `panon a to ka nya bu pegkanen na ngen i naluto bo a karne`,
      `panon a to nginto ka di mapya i kakan sa da maluto a seda o karne`,
      `ngen sabap nin ka di mapya keman sa da maluto a karne su taw`,
      `enduken a to ka di kanen i nganin nganin a da maluto a karne`,
    ...
    encodeMemory([
      ...
      `ngin i enggulan nu taw para di sekanin edtagudo`,
      `ngin i galbeken nu taw endu di sekanin edtagudo`,
      `panon i kadtagudo nu taw`,
...
let section7 = [
   ...
   encodeMemory([
    `* (ngin|duken) * (inom|kan) * diarrhea_foods_allowed *  tagudo *`, 
    `* (ngin|duken) * (pamasa) * diarrhea_foods_allowed * tagudo *`, 
    `* (ngin|duken) * (inom|kan) * diarrhea_foods_allowed * tagudo *`, 
    `* diarrhea_foods_allowed * (inom|kan) * tagudo *  `, 
    `* diarrhea_foods_allowed * (mapya|tabang|kapakay) * tagudo *`, 
    `ngin to ka day a keman o minom  su taw sa diarrhea_foods_allowed upama ka pedtagudo`,
    `mapakay a keman o minom sa diarrhea_foods_allowed i pedtagudo`,
   ],[
   ...
   encodeMemory([
    ...
    `ngin i ipasepa sa taw a pedtagudo`,
    `ngin i mapakay a kanen nu taw a pedtagudo`,
    `ngin i mga mapya a kanen nu taw a pedtagudo`,
    `ngin i mga pegken a kapakay a pamasan o kanen nu taw a pedtagudo`,
...
let section8 = [
    encodeMemory([
      `* (nginto|duken) * diarrhea_foods_not_allowed_bad_hydration * di * (inom|kan|pamasan) * tagudo *`, 
      `* (nginto|duken) * di * (inom|kan|pamasan) * diarrhea_foods_not_allowed_bad_hydration * tagudo * `,  
      `nginto ka di pakanen su taw sa diarrhea_foods_not_allowed_bad_hydration upama pedtagudo`,
      `ngin to ka temangka su taw sa diarrhea_foods_not_allowed_bad_hydration amayka pakagedam sa kapedtagudo`,
      `mapakay a keman o minom su taw a pedtagudo sa diarrhea_foods_not_allowed_bad_hydration`,
      `kapakayan a keman o minum sa diarrhea_foods_not_allowed_bad_hydration i taw a pedtagudo`,
      `ngin i mapya a galbeken nu taw amayka aden diarrhea_foods_not_allowed_bad_hydration`,
      `enduken ka di mamasa sa diarrhea_foods_not_allowed_bad_hydration i tayn san a pedtagudo`,
      `ngin sabap nin ka , da ka pamasa sa diarrhea_foods_not_allowed_bad_hydration upama ka pedtagudo su taw`,
    ],[
    ...
    encodeMemory([
      ...
      `nginto ka di pakanen su taw sa diarrhea_foods_not_allowed_contains_lactose upama pedtagudo`,
      `ngin to ka temangka su taw sa diarrhea_foods_not_allowed_contains_lactose amayka pakagedam sa kapedtagudo`,
      `mapakay a keman o minom su taw a pedtagudo sa diarrhea_foods_not_allowed_contains_lactose`,
      `kapakayan a keman o minum sa diarrhea_foods_not_allowed_contains_lactose i taw a pedtagudo`,
      `ngin i mapya a galbeken nu taw amayka aden diarrhea_foods_not_allowed_contains_lactose`,
      `enduken ka di mamasa sa diarrhea_foods_not_allowed_contains_lactose i tayn san a pedtagudo`,
      `ngin sabap nin ka , da ka pamasa sa diarrhea_foods_not_allowed_contains_lactose upama ka pedtagudo su taw`,
    ...
    encodeMemory([
      ...
      `enduken ka di mamasa sa diarrhea_foods_not_allowed_irritate_stomach i tayn san a pedtagudo`,
      `kapakayan a keman o minum sa diarrhea_foods_not_allowed_irritate_stomach i taw a pedtagudo`,
      `mapakay a keman o minom su taw a pedtagudo sa diarrhea_foods_not_allowed_irritate_stomach`,
      `ngin i mapya a galbeken nu taw amayka aden diarrhea_foods_not_allowed_irritate_stomach`,
      `ngin sabap nin ka , da ka pamasa sa diarrhea_foods_not_allowed_irritate_stomach upama ka pedtagudo su taw`,
      `ngin to ka temangka su taw sa diarrhea_foods_not_allowed_irritate_stomach amayka pakagedam sa kapedtagudo`,
      `nginto ka di pakanen su taw sa diarrhea_foods_not_allowed_irritate_stomach upama pedtagudo`,
    ],[
    ...
    encodeMemory([
      `* duken * di * (inom|kan|pamasan) * diarrhea_foods_not_allowed_oily * tagudo *`, 
      `* duken * diarrhea_foods_not_allowed_oily * di * (inom|kan|pamasan) * tagudo *`, 
      `enduken ka di mamasa sa diarrhea_foods_not_allowed_oily i tayn san a pedtagudo`,
      `kapakayan a keman o minum sa diarrhea_foods_not_allowed_oily i taw a pedtagudo`,
      `mapakay a keman o minom su taw a pedtagudo sa diarrhea_foods_not_allowed_oily`,
      `ngin i mapya a galbeken nu taw amayka aden diarrhea_foods_not_allowed_oily`,
      `ngin sabap nin ka , da ka pamasa sa diarrhea_foods_not_allowed_oily upama ka pedtagudo su taw`,
      `ngin to ka temangka su taw sa diarrhea_foods_not_allowed_irritate_stomach amayka pakagedam sa kapedtagudo`,
      `nginto ka di pakanen su taw sa diarrhea_foods_not_allowed_irritate_stomach upama pedtagudo`,
    ],[
    ...
    encodeMemory([
      ...
      `labit ka sa laki ngin i mga pegken a di mapakay kanen nu taw a pedtagudo`,
      `ngen a mga di mapya a kanen nu taw a pedtagudo`,
      `ngen a mga pegken i di pamasen nu pedtagudo`,
      `ngen i makalunsan a pegken sa taw a pakagedam ka kapedtagudo`,
      `ngen i mga di inumen nu taw a pedtagudo`,
      `ngen i pegken a makalunsan ka kapedtagudo`,
      `ngin i mga pegken a di dayt a pamasan kanen o iluton nu taw a pedtagudo`,
...
let section10 = [
    encodeMemory([
      ...
      `Ngin a mga taw i mangingat sa kapedtagudo`,
