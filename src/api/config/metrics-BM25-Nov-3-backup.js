  ...
  getScore(input){
    ...
    if ( documentScores[0] == undefined ){
      documentScores[0] = {
        score: 0,
        index: -1,
        pattern: "metrics::not found",
        rawPattern: "metrics::not found"
      }
    }
    ...
    return {
      ...
      rawPattern: currentMax.rawPattern
    }
  }
...
//     // BM25+
  ...
  calculateBM25(term, doc) {
      ...
      // read this for bm25+ 
      // https://dl.acm.org/doi/abs/10.1145/2063576.2063584
      // https://sci-hub.se/https://dl.acm.org/doi/abs/10.1145/2682862.2682863
      let toAvoidZeroDivision = .5
      // idf = Math.log((N + 1 + toAvoidZeroDivision) / (documentContainingTermLength  + toAvoidZeroDivision) );
      // by smoothing it with .5 value / we add this to the numerator and denominator
      ...
      // if ( _max_iteration > 0){
      //   _max_iteration--     
      //   console.log(`Metrics::Sentence >> ${doc}`)
      //   console.log(`Metrics::term >> ${term}`)
      //   console.log(`Metrics::tf + sigma >> ${tf+this.sigma}`)
      //   console.log(`Metrics::idf >> ${idf}`)
      // }
